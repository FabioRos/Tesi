...
\hl{Nuovo capitolo}
	...
	Tutte le figure di sistema presentano requisiti analoghi. Per evitare di affaticare la lettura con sezioni ridondanti è stato scelto di riportare soltanto il diagramma dei casi d'uso riguardante gli RSPP. Si intende che per tutte le altre figure di sistema il diagramma sia analogo. 
		...
		\begin{itemize}
			...
			\begin{itemize}
				...
				\item \textit{L'utente autenticato viualizza il questionario associato ad un RSPP (UC1.1.4);}
	...
	\subsubsection{UC4.2 Gestione delle formazioni dei dipendenti e del datore di lavoro }
		...
		\begin{itemize}
			...
			\begin{itemize}
				...
				\item \textit{L'utente autenticato inserisce gestisce il questionario relativo ad una formazione in possesso di un membro dell'azienda (UC4.2.6).}
			\end{itemize}
		\end{itemize}
	...
	\subsection{UC6 Gestione procedure}
		...
		Le procedure si prassi derivano direttamente da un documento fornito dall'\gls{INAIL}. Queste procedure sono state individuate dall'analisi delle cause di infortunio maggiormente frequenti rilevate dall'\gls{INAIL} nel tempo allo scopo di migliorare la sicurezza dei lavoratori.
		Le procedure di sistema, invece, sono stese dall'azienda e fanno parte del \gls{DVR}. 
		\begin{figure}[H]
			\begin{center}
		...
		\end{figure}
			...
			\begin{itemize}
				...
				Le procedure di prassi sono state gestite con un questionario (vedi sezione: \ref{section:UC5}) in quanto derivanti direttamente da una lista di domande proveniente dall'\gls{INAIL};
				\item \textbf{Attori Coinvolti:} Utente Autenticato;
				\item \textbf{Flusso principale degli eventi:} 
				\begin{itemize}
					\item \textit{L'utente autenticato gestisce delle procedure di sistema(UC6.1);}
					\begin{itemize}
					...
					\end{itemize}
					\item \textit{L'utente autenticato gestisce delle procedure di prassi (UC6.2);}
					\begin{itemize}
